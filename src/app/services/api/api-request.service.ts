...
export class ApiRequestService {
  ...
  download(url:string, params?: HttpParams):Observable<any>{
    let me = this;
    return this.http.get(this.appConfig.baseApiPath + url, {responseType: 'blob', headers : this.getHeadersForUpload(), params : params})
      .catch(function(error:any){
        console.log("Some error in catch");
        if (error.status === 401 || error.status === 403){
          me.router.navigate(['/401']);
  ...
  post(url:string, body:Object, params?: HttpParams):Observable<any>{
        let me = this;
        return this.http.post(this.appConfig.baseApiPath + url, JSON.stringify(body), { headers:this.getHeaders(), params: params})
            .catch(function(error:any){
                if (error.status === 401 || error.status === 403){
                    me.router.navigate(['/401']);
                }
